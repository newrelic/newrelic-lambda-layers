...
tap.test('CJS Edge Cases', (t) => {
  ...
  t.beforeEach(() => {
    ...
    ;({ handler } = proxyquire('../../callback-wrapper', {
      'newrelic': newrelic
    }))
  })
  ...
  t.test('should throw when NEW_RELIC_LAMBDA_HANDLER is missing', (t) => {
    t.rejects(
      async () => await handler({ key: 'this is a test'}, { functionName: 'testFn'}),
  ...
  t.test('should throw when NEW_RELIC_LAMBDA_HANDLER module cannot be resolved', async(t) => {
    ...
    t.rejects(
      async () => await handler({ key: 'this is a test'}, { functionName: handlerMethod }),
